...
max_err={'temperature':4,'waterVapor':2}#maximum 95% c.i. width
...
ticks={'temperature':np.arange(10,22.1,0.5),'waterVapor':np.arange(4,9.1,0.25)}
colormaps={'temperature':'coolwarm','waterVapor':'Blues'}
...
ctr=1
for v in variables:
    ax=fig.add_subplot(1,len(variables),ctr)
    cf=plt.contourf(hour_avg,Data_avg.height*1000,Data_avg[v].T,ticks[v],cmap=colormaps[v],extend='both')
    plt.contour(hour_avg,Data_avg.height*1000,Data_avg[v].T,ticks[v],colors='k',linewidths=0.5,linestyles='solid',alpha=0.5)
